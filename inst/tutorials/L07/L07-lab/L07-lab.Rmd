...
1. 由于本 Rmd 文档将未完成的代码块的 `eval` 参数设置为 `FALSE`，文档可直接 Knit
（快捷键为 `Ctrl+Shift+K`）；在同学们根据提示逐个补充完善代码块并运行无误后，应将 `eval` 参数设置为 `TRUE`，Rmd 文档才会 Knit 出最终的结果。

2. 本次编程实操练习为 “L07 迭代” 一讲的配套课后练习。
...
3. 尽管 `readr` 包 `v2.0.0` 之后直接支持读入并合并一个文件夹下多个同质的纯文本文档（代码如下），但我希望同学们不要直接走捷径，而是能循序渐进完成以下练习`#1-#3`——这有助于你们加深对本讲内容的理解和掌握。
...
# 若执行无误，记得将代码块选项 eval=FALSE 修改为 eval=TRUE，然后 Knit
...
```{r ex1-2, eval=FALSE}
# 将 <...> 替换为合适的代码，其余代码勿动，参考 ex1-1 代码块
# 修改后点击代码块右上侧的绿色三角按键或使用快捷键 Ctrl+Shift+Enter 执行代码块
...
之前的 `read_one_csv()` 函数体感觉不够紧凑和流畅，在此我们用管道操作符 `%>%` 将 `read_one_csv()` 修改写为函数 `read_one_csv2()`。
...
完成上述练习#1-#3后记得将下面代码块中的 `eval=FALSE` 修改为 `eval=TRUE`，重新 Knit 文档，看你能否得到一个大大的 `yeah`？！

```{r, echo=FALSE, eval=FALSE}
{
  stopifnot(
    # out_tb、out_tb2和out_tb3 包含不同的属性，例如相比
    # out_tb3，out_tb 还额外包含"spec_tbl_df"类和"spec"属性
    # 用[]选择子集操作以去除属性
    identical(out_tb[], out_tb2[]),
    identical(out_tb[], out_tb3[])
  )
...
尽管上述三个练习都能得到相同的结果，但复杂程度有所不同。回过头去认真读一下你写下的代码，你最喜欢 &#x1F60D; 哪种操作呢？
