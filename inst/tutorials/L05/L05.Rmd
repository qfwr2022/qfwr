...
以下选择题主要用来测试同学们对本讲所涉及基础概念和核心函数的理解和掌握情况——
这是熟练运用各种函数完成数据处理任务的基本功 `r emo::ji("muscle")`。

```{r quiz}
quiz(
  question("1 构成 `dplyr` 包数据处理基本操作的函数包括：",
           ...
           correct = random_praise_zh(), # "正确！",
           incorrect = "错误！参见 _R数据科学_ 的3.1.3小节。",
           allow_retry = TRUE,
           random_answer_order = TRUE
  ),
  question("2 以下哪些函数是对数据表的行（样本）进行操作的？",
           answer("`filter()`", correct = TRUE),
           answer("`arrange()`", correct = TRUE),
           answer("`mutate()`"),
           answer("`transmute()`"),
           ...
           correct = random_praise_zh(), # "Great！",
           incorrect = random_encouragement_zh(), # "错误，请重试！",
           allow_retry = TRUE,
           random_answer_order = TRUE
  ),
  question("3 以下哪些函数与数据表的列（变量）操作有关？",
           answer("`filter()`"),
           answer("`arrange()`"),
           answer("`mutate()`", correct = TRUE),
           ...
           correct = random_praise_zh(), # "Excellent！",
           incorrect = random_encouragement_zh(), # "错误，请重试！",
           allow_retry = TRUE,
           random_answer_order = TRUE
  ),
  question("4 以下关于 `dplyr` 包中函数说法正确的有：",
           answer("在 `summarise()` 中较少使用<i>N</i>元素进<i>N</i>元素出的
                  向量化的函数，如 `log()`。", correct = TRUE),
           answer("`mutate()` 总是将新变量添加在数据集的最前面。"),
           answer('`transmute()` 仅保留新变量和分组变量，其效果类似于设定参数 
                  `.keep = "none"` 的 `mutate()` 函数。', correct = TRUE),
           answer("`select()` 可以在选择变量的同时对变量进行重命名。", 
                  correct = TRUE),
           answer("`group_by()` 生成分组数据集，这会对后续步骤的数据操作产生
                  影响，必要时可用 `ungroup()` 来取消分组设定。", 
                  correct = TRUE),
           correct = random_praise_zh(), # "漂亮！",
           incorrect = random_encouragement_zh(), # "错误，请重试！",
           allow_retry = TRUE,
           random_answer_order = TRUE
  ),
  question("5 以下关于多表操作说法正确的有：",
           answer("在数据处理实践中最常用到的合并式连接函数为 `left_join()`。", 
                  correct = TRUE),
           answer("`semi_join()` 和 `anti_join()` 完成的是样本筛选式连接。",
                  correct = TRUE),
           answer("`*_join()` 族函数的 `on` 参数用来设定连接所用的键变量，
                  支持灵活多样的设定。", 
                  message = "是 `on` 还是 `by`？这是个问题！&#x1F914;"),
           answer("`bind_cols()` 基于行序对多个数据集进行横向合并，
                  无需指定键变量。", correct = TRUE),
           answer("`bind_rows()` 基于变量名对多个数据集进行纵向合并。", 
                  correct = TRUE),
           correct = random_praise_zh(), # "答对了！给你点赞！",
           incorrect = random_encouragement_zh(), # "错误，请重试！",
           allow_retry = TRUE,
           random_answer_order = TRUE
  ),
  question("6 以下关于 `dplyr` 包优点概括正确的有：",
           answer("`dplyr` 以一组清晰易记的动词函数来完成常见的数据处理任务。", 
                  correct = TRUE),
           answer("`dplyr` 核心函数的第一个参数为数据集，返回值也为数据集，
                  这使得它能很好地支持管道操作 `%>%`。", correct = TRUE),
           answer("`dplyr` 可以通过接口包（如 `dbplyr` 和 `dtplyr`）以相同的
                  动词函数来操作其他常见的数据库（如 `SQLite`）和 R 数据结构（如 
                  `data.table`）。", correct = TRUE),
           answer("`dplyr` 用 `C++` 实现很多关键的底层代码，从而完成对内存中
                  数据的高效运算。", correct = TRUE),
           correct = random_praise_zh(), # "Bingo！",
           incorrect = random_encouragement_zh(), # "错误，请重试！",
           allow_retry = TRUE,
           random_answer_order = TRUE
  ),
...
接下来需要同学们循序渐进地完成6个简单的编程练习。

### EX01

#### 练习1. 了解数据集

我已经提前用 `library()` 函数帮同学们载入代码练习需用到**`dplyr`**包、
**`ggplot2`**包以及**`nycflights13`**数据包。

首先，请同学们在下面**代码窗口**键入相关 R 函数来大致了解相关的数据集，并熟悉
`learnr` 代码输入窗口的基本操作。
...
```{r ex1-hint-5}
View(flights)  # 在数据浏览窗口中打开flights数据集，反应速度会有点慢
```

接下来我们分几个小练习，逐一了解下 `dplyr` 包基本的数据操作函数。
...
筛选出`flights`数据集中出发延误至少1小时，但在飞行过程中追回至少30分钟的航班。

```{r ex2, exercise=TRUE, exercise.lines=5, exercise.reveal_solution=TRUE}
...
3. ！！！完成代码后，记得点击`Submit Answer`检查答案正确与否并提交答案；
& ...
...
```{r ex2-check}
nc_msg <- "`dplyr`包中用来筛选样本的函数是`filter()`，你选对了吗？"
...
success_msg(random_praise_zh(code = TRUE))
```
...
```{r ex3-check}
nc_msg  <- "尽管有其他实现方式，但最适合解决这个问题的函数就是用 `dplyr` 包中 `select()` 函数来选取（并重命名）变量，你选对动词函数了吗？"
udf_msg <- "你忘记将 `select()` 函数返回结果存为新数据集 `flights_sml` 了？"
inc_msg <- "除了题目要求你们选择的5个变量之外，你还_额外_多选了几个变量？"
cm_msg  <- "你可能忘记选取并按要求_重命名_变量 year、month 或 day 了？"
cm_msg2 <- "你可能忘记_选取_变量 dep_time 或 dep_delay 了？"
...
```{r ex4, exercise=TRUE, exercise.lines=6, exercise.reveal_solution=TRUE}
flights
...
1. 不难看出，这些变量的后两位数字为分钟，之前的一至两位数字为小时，
   即1401表示14:01，想想有什么好办法可将它们拆解合并为分钟数。
```
...
```{r ex4-check}
nc_msg  <- "你选用什么函数呢？`dplyr`包中生成新变量/修改变量的大杀器就是`mutate()`！"
udf_msg <- "你忘记将`mutate()`函数返回结果存为新数据集`flights_new`了？"
...
success_msg(paste0(random_praise_zh(code = TRUE), 
                   "几乎相同的代码重复了四次，我疏忽出错两次，你呢？"))
```
...
```{r ex5, exercise=TRUE, exercise.lines=6}
by_day <- ____(flights, ____, ____, ____)
mdelay_by_day <- ____(by_day, mdelay = ____)
mdelay_by_day
...
```{r ex5-check}
udf_msg1g <- "找不到数据集 by_day ——请使用 by_day 变量保存分组后的数据集。"
inc_msg1g <- "group_by() 函数的 .data 参数设置有误。"
...
udf_msg1s <- "找不到数据集 mdelay_by_day ——你忘记将生成的汇总数据保存为该数据集了？"
...
下图所示为2013年6月13日航班平均**到达**延误时间的空间分布情况：
...
ex() %>% check_function(., "group_by") %>% {
  check_arg(., "...") %>% 
    check_equal(eval = FALSE, append = FALSE, 
                incorrect_msg = "要按目的地城市变量`dest`进行分组。")
}

ex() %>% check_function(., "summarise") %>% 
  check_result(.) %>% 
  check_equal(incorrect_msg = "平均延误时间的计算结果有误，请仔细检查下代码。", 
              append = FALSE)
...
nc_msg <- "作图部分的代码就不用折腾修改啦，直接保留代码模板中给出的代码即可。"
ex() %>% {
...
```{r ui, echo=FALSE}
qfwr:::submission_ui(cid = "689dc88a5f7c473ebd06c9845c5afe1f")
```
