...
quiz(
  ...
  question("1.2 如果一个文本文档中各字段用`|`分隔，那么应该使用哪个函数来读取
    ...
    correct = paste0(random_praise_zh(), '`read_delim()`可通过设置参数`delim = "|"`来读取用`|`分隔的文本文档。'),
    incorrect = NULL,
    allow_retry = TRUE, 
  ...
  question("1.3 想一想执行以下哪些命令会出现有关“解析问题（ _parsing 
    ...
    incorrect = paste0(random_encouragement_zh(), "你可能需要在 R 控制台中输入代码逐一试看看。"),
    allow_retry = TRUE, 
    random_answer_order = TRUE
  ),
  ...
  question("1.4 以下关于`readxl::read_excel()`函数说法正确的有哪些？",
    ...
    answer("`range`参数具体指定读入的单元格区域，当和`sheet`、`skip`、
          `n_max`等参数的设定发生冲突时具有更高优先级。", correct = TRUE),
  ...
  question("1.5 除了使用`readr`包和`readxl`包来读入文本表格和 Excel 表格数据之外，R 
    ...
    answer("若同学们手头有文本数据、网络关系数据、图像数据、基因组数据等需要读入
           R中进行后续处理，比较推荐的方式是到 [CRAN Task Views](https://cran.r-project.org/web/views/) 上了解具体有哪些包可供使用。", correct = TRUE), 
    correct = paste0(random_praise_zh(), 
                     "本题的最主要目的在于让同学们初步了解下 R 
                     读入形形色色数据的可能性空间（néngnài）有多大。"), 
    incorrect = "多选题，请重试。",
    allow_retry = TRUE, 
    random_answer_order = TRUE
  ),
...
quiz(
  question("2.1 以下关于`tibble`对象和传统数据框`data.frame`的说法正确的有：",
    answer("`tibble`对象是 R 传统数据框`data.frame`的一种变体，它是支撑
          ...
          message = "`tibble`对象最低一层的class为\"data.frame\"，因此，
                    那些用来处理`data.frame`类的函数也可直接应用于`tibble`对象。"),
    ...
    answer("可用`$`和`[[`来提取`tibble`对象中的某一列（变量），但`tibble`对象
           不支持列名的部分匹配。", correct = TRUE), 
    correct = random_praise_zh(), 
    incorrect = "答错啦！请认真阅读_R for Data Science_的[第10章](https://r4ds.had.co.nz/tibbles.html)、相关 R 包的 
  ...
  question("2.4 `tidyr`包中提供的对数据集中**显性**缺失值`NA`的处理函数包括：",
    ...
    correct = paste0(random_praise_zh(), 
                     "尽管我们上课时由于时间限制选择跳过，但这些函数
                     还是值得/需要同学们花点时间了解掌握滴。"), 
    incorrect = paste0(random_encouragement_zh(), "请查看相关函数的帮助文档。"),
    allow_retry = TRUE,
    random_answer_order = TRUE
  ),
  caption = "**不定项选择题**"
...
2. 两份数据出处及说明如下：
    ...
    - `ipos_profile.csv` 数据来自 “Wind>股票>沪深股市专题统计>一级市场>IPO>新股
        ...
        为了减小qfwr包的大小，我特意把它压缩为ipos_profile.zip文档）
...
5. <span style='color: red'><strong>每个练习 `▶ Run Code` 无误后务必记得点击 `☑ Submit Answer`。</strong></span>
...
——同学们就先按这个基本思路来处理数据，暂时不要即兴发挥啦。`r emo::ji("stuck_out_tongue_winking_eye")`
...
# x. 通常是不需要以下这个步骤，在此仅仅为了方便编写后续的leanr练习
...
<br>在此练习中我们使用 **`readxl`包** 来读入 .xlsx 或 .xls 格式的文档，基本的套路还和前一练习类似，但同学们需要查看下所用函数的具体参数
设置（`?read_excel`）。
...
后续分析的影响并不大，也可以通过`lubridate::as_date()` 转为 `date` 类型）。
...
```{r ui, echo=FALSE}
qfwr:::submission_ui(cid = "4315cfa7534a4414ade8a384740c85d9")
```
