#' qfwr: Interactive tutorials and helper tools for QFwR course.
#'
#' @description
#' Interactive tutorials and helper tools for my course _Quantitative Finance 
#' with R_ to facilitate teaching and learning. The interactive exercises are 
#' mainly powered by RStudio's **learnr**, Datacamp's **testwhat** and some 
#' ideas stolen from Prof. Rundel's **learnrhash**.
#'
#' @author
...
#'
#' Run an interactive exercise which is contained within the R package 
...
#'
#' List all interactive exercises which are contained within the
...
qfwr_lab <- function(lecture = NULL) {
  ...
  if(isAvailable()) {
    ...
    stop("`qfwr_lab() can only be called within RStudio IDE.")
  }
...
#'
#' @param lecture  An string, such as "L06".
